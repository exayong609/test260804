...
export async function processImportBatch(payload: ImportBatchPayload) {
  ...
  try {
    ...
    if (result.applied && insertDurationMs > 0) await recordInsertDuration(payload.task_id, payload.unit_id, insertDurationMs);
    return { ...result, degraded, insertDurationMs };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
